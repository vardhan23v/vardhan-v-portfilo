...
function StackSpreadStage({
  ...
  showScrollHint = true,
  stickyHeight,
}: StackSpreadStageProps & { stickyHeight?: number }) {
  ...
  return (
    ...
    >
      <div
        className={`sticky top-0 w-full overflow-hidden${stickyHeight == null ? " h-screen" : ""}`}
        style={stickyHeight != null ? { height: stickyHeight } : undefined}
      >
        {/* centre text */}
        <motion.div
...
export interface StackSpreadProps {
  ...
  /** show the "scroll to spread" hint at the bottom until the scatter begins */
  showScrollHint?: boolean;
  /** desktop-only: explicit sticky viewport height in px (defaults to 100vh). Use when the scroll context is not the window, e.g. an embedded panel. */
  stickyHeight?: number;
}
